...
| Fotos de instructores | `images/Profesores/Cipri.webp`, `Boris.webp`, `Marta_Pozo.webp` (WebP, no PNG) | Puestas a mano en el HTML de las dos webs. Renovadas el 6 de septiembre de 2026 con fotos de estudio (fondo blanco, logo Itaca en la pared) que mandó Cipri por chat; recortadas de horizontal a 3:4 centrando a cada persona — el archivo de Marta ya no es el horizontal de baja resolución que arrastraba desde el principio. Recentradas el 7 de septiembre: el primer recorte de Boris y Marta salió descuadrado (se cortaba tomando como centro un punto a su derecha y quedaban desplazados hacia la izquierda de la ficha). Al recortar de un original horizontal, el centro del recorte tiene que caer sobre el cuerpo de la persona, no sobre el centro de la foto. |
...
⚠️ **El reparto se comprueba al cargar Y al cambiar el ancho de la ventana** (`matchMedia(...).addEventListener('change', …)` en `movil.html`, añadido el 7 de septiembre de 2026). Antes solo miraba al cargar: quien agrandaba la ventana —o cambiaba el tamaño en las herramientas del navegador, que es como se detectó— se quedaba en `movil.html` para siempre viendo la versión de móvil estirada a 1900px. La vía de escape `?movil=1` sigue mandando por encima de esto: con ella no salta aunque se agrande.

⚠️ **`movil.html` tiene un tope de anchura de 560px centrado por encima de 620px** (`main`, `.foot`, `.panel__bar`, `.panel__body`; la barra pegajosa `.top` mantiene el fondo de borde a borde y centra su contenido con `padding-inline`). Es una red de seguridad, no el modo normal de verla: existe porque el reparto puede no ejecutarse (JavaScript desactivado, `?movil=1` a propósito). Sin ese tope el contenido se estiraba de lado a lado con el margen fijo de 22px y parecía la web rota — que es exactamente lo que reportó una revisión de diseño externa, confundiéndolo con la web de escritorio.
