...
- **Framework:** ninguno. HTML, CSS y JavaScript a pelo. **Una sola web para todos**, `index.html` (~4.700 líneas), responsive de verdad. Usa dos módulos JS y las páginas legales aparte — ver §3. Hasta septiembre de 2026 hubo una segunda web solo para móviles (`movil.html`); se retiró (ver §6).
...
| Sistema visual del panel | `docs/sextante.css` | **«Sextante»**, el sistema visual de los paneles internos (el panel de solicitudes está hecho con él). Hoja de estilos portable, sin dependencias, para reutilizar en cualquier otro cuadro de mando. Sus tres reglas: esquina viva en todo (`--radio-cero: 0`), monoespaciada para lo que se mide y de display para lo que se lee, y el color solo informa (verde en marcha, ámbar atención, rojo se enfría, cian dato). Hay una referencia navegable con vistas previas en vivo en el sistema de diseño «Sextante» de los artefactos de Cipri. **No se usa en la web pública**, que es editorial y clara: Sextante es solo para paneles internos. |
| Script de Google (hoja + panel) | `docs/apps-script.js` | **Fuente de verdad del script.** Lo publicado en Apps Script tiene que ser este archivo entero. Trae `doGet` (lo que lee y guarda el panel), `doPost` (lo que escribe la web al enviar un formulario) y `comprobar()` para validarlo desde el editor antes de publicar. |
...
| Fotos del gimnasio | `images/Itaca/Gym1.webp` … `Gym12.webp` (12) | Las lee el array `photos` del JS de `index.html`. En pantalla ancha solo las 8 primeras salen en el carrusel (`CAROUSEL_MAX`); todas salen en "Ver todas las fotos". `Gym8-12.webp` son nuevas (6 de septiembre de 2026, fotos de Drive/chat recortadas a 1200×800; `Gym9`, `Gym10` y `Gym11` llevan cara de una menor con permiso firmado de la familia, confirmado por Cipri, que es el padre). |
| Fotos de instructores | `images/Profesores/Cipri.webp`, `Boris.webp`, `Marta_Pozo.webp` (WebP, no PNG) | Puestas a mano en el HTML. Renovadas el 6 de septiembre de 2026 con fotos de estudio (fondo blanco, logo Itaca en la pared) que mandó Cipri por chat; recortadas de horizontal a 3:4 centrando a cada persona — el archivo de Marta ya no es el horizontal de baja resolución que arrastraba desde el principio. Recentradas el 7 de septiembre: el primer recorte de Boris y Marta salió descuadrado (se cortaba tomando como centro un punto a su derecha y quedaban desplazados hacia la izquierda de la ficha). Al recortar de un original horizontal, el centro del recorte tiene que caer sobre el cuerpo de la persona, no sobre el centro de la foto. |
| Portada (única, todas las pantallas) | vídeo `videos/Itaca_Hero_Desktop.mp4`, con `images/Itaca/Portada-escritorio.jpg` como respaldo/poster | Vuelve a ser vídeo (6 de septiembre de 2026): el anterior (`Itaca_Hero_Video.mp4`) era una animación de logo casi en negro y se borró; este es metraje real del gimnasio, 1920×800, sacado de Drive. Arranca silenciado (ningún navegador deja sonar un vídeo solo) y tiene un botón de sonido (`.video-control`) para activarlo a propósito, visible también en el móvil. ⚠️ Es horizontal (1920×800): en un teléfono `object-fit: cover` recorta bastante por los lados. Es la contrapartida asumida de tener un solo vídeo. |
| Vídeo vertical retirado | `videos/Itaca_Hero_Mobile.mp4` | **Ya no se usa.** Fue la portada de móvil hasta el 8 de septiembre de 2026. Se retiró porque Cipri quiere un solo vídeo: *"no quiero dos videos diferentes, debe haber solo 1, el original"*. El archivo se conserva por si algún día se recupera; no se borra. |
| Vídeos verticales (reels) | `videos/Boris_instagram.mp4`, `Cipri_instagram.mp4`, `Marta_instagram.mp4`, `Itaca_gi_instagram.mp4` | `preload="none"`: no se descargan hasta que hace falta. Con ratón se reproducen al pasar por encima. **Donde no hay ratón se lanzan solos**: un `IntersectionObserver` reproduce en silencio los que se ven más de la mitad, **dos como máximo** (entre los cuatro pesan 18 MB y casi todo el mundo entra con datos), y los para en cuanto salen de pantalla; hay un segundo observador sobre `#reels` como red de seguridad, porque con un scroll rápido el primero puede no disparar y quedaría un vídeo sonando fuera de pantalla. Para oírlos se toca la ficha y se abre el visor a pantalla completa (`#reelsViewer`). Se respeta `prefers-reduced-motion`. Entre los cuatro pesan 18 MB, por eso `preload="none"`. |

⚠️ **El panel y el script de Google: lo que costó media docena de despliegues en septiembre de 2026.** El panel decía "no se ha podido conectar" y se fueron probando cambios a ciegas. Eran tres fallos a la vez, y ninguno era el que parecía:

1. **El script publicado había perdido `doPost`.** Se habían pegado versiones parciales que solo traían `doGet`. Sin `doPost`, la web dejó de guardar las solicitudes en la hoja — y como el correo de Web3Forms seguía llegando, **nadie se enteró** — y el panel no podía marcar, archivar ni anotar nada. Por eso `docs/apps-script.js` existe ahora: para que el script se pegue entero o no se pegue.
2. **El JSONP no puede funcionar con Apps Script.** Google sirve la respuesta con `X-Content-Type-Options: nosniff`; en cuanto el tipo de contenido no es exactamente JavaScript (y con `MimeType.TEXT` no lo es), el navegador **se niega a ejecutar** el `<script>` y solo se ve el error genérico de conexión. **No vuelvas a JSONP.**
3. **No hacía falta JSONP para nada.** Las respuestas GET de Apps Script llevan `Access-Control-Allow-Origin: *`, así que un `fetch()` normal funciona desde `www.itacajiujitsu.com`. El panel ahora usa GET + `fetch` para todo, también para guardar.

**La lección de método, que es la que importa:** el fallo se encontró en dos minutos con un `curl -i -L` a la URL del script, mirando las cabeceras y el cuerpo de la respuesta. Antes de eso se habían hecho seis despliegues adivinando. **Ante un servicio externo que falla, pídele una respuesta y léela entera antes de tocar una sola línea de código.**

⚠️ **El `poster` de la portada es el primer fotograma exacto de su vídeo** (`Portada-escritorio.jpg`). No lo cambies por "una foto bonita" cualquiera: mientras el vídeo carga se ve el poster, y si no coincide con el primer fotograma se ve un salto feo el primer instante — que es justo el fallo que reportó Cipri el 7 de septiembre de 2026. Si cambias un vídeo de portada, regenera su poster: `ffmpeg -i <video> -frames:v 1 <poster>.jpg`.
...
**Una sola web para todos.** No hay reparto ni redirección por ancho de pantalla: `index.html` se sirve igual en un teléfono y en un monitor, y se adapta con media queries. Se decidió el 7 de septiembre de 2026, a petición de Cipri: *"yo quiero ver en el móvil la versión completa"*.

Lo que había antes: `index.html` mandaba a `movil.html` por debajo de 760px y `movil.html` devolvía por encima de 900px, con `?full=1` y `?movil=1` como vías de escape guardadas en `sessionStorage`. Todo eso se ha retirado; `movil.html` es ahora un reenvío de 50 líneas que manda a `index.html` conservando el ancla, para que los enlaces antiguos no den 404.
...
⚠️ **El `<meta name="viewport">` es `width=device-width`, y no se toca.** Durante unas horas del 7 de septiembre de 2026 fue `width=980`, para que el teléfono enseñara la versión de tablet empequeñecida (Cipri había grabado la web en "modo ordenador" y le gustaba así). Se revirtió al día siguiente: esa línea **apagaba de golpe todos los cortes por debajo de 1024px** y con ellos la letra legible sin zoom, la barra inferior de acciones y el vídeo vertical con sonido; además Google podía marcar la web como no apta para móviles, que es justo lo que penaliza en "jiu jitsu Logroño". Si alguien vuelve a pedir "que en el móvil se vea como en el ordenador", la respuesta no es el viewport: es pulir el acabado de la versión de móvil (ver abajo).

⚠️ **La versión de móvil va a esquina viva y a sangre, a propósito** (bloque «Móvil con carácter editorial», el último del `<style>`). Lo que hacía que a Cipri le gustara la de escritorio y no la de móvil no era el contenido: era el acabado. En el móvil todo llevaba esquinas redondeadas (fichas 14px, vídeos 16px, fotos 18px) y pestañas de día en forma de píldora, flotando con huecos sobre el fondo — lenguaje de aplicación. La editorial va a esquina viva, de borde a borde y con los bloques tocándose. **No vuelvas a meter `border-radius` en móvil** pensando que es un descuido.

⚠️ **`.footer-simple .footer-links` necesita `flex-wrap: wrap`.** Los cuatro enlaces del pie miden 447px en una fila y no caben en un teléfono: sin el wrap se salían por los dos lados y aparecía **barra de scroll horizontal en toda la página por debajo de 480px**. Llevaba ahí desde siempre y no lo causó ningún cambio reciente; se detectó midiendo `document.documentElement.scrollWidth` contra `innerWidth` a 320/360/390/430px.

⚠️ **Para medir anchos de verdad, no uses `isMobile: true` en Playwright.** Con esa opción el ancho CSS que maqueta el navegador **no es el que pides**: pides 390 y maqueta a 427 (un 9,5% más). Eso hace que una prueba de "cabe a 320px" pase cuando en realidad se probó a 350. Para medir, viewport a secas; `isMobile` solo para comprobar comportamiento táctil.
...
- El **poema** se partía en teléfonos estrechos: el verso más largo mide 365px y la columna se queda en ~350px a 390px de ancho. Se arregló con `font-size: clamp(0.82rem, 4.2vw, 1.13rem)` en `.poem-body`. Comprobado: 13 versos en 13 líneas de 320px a 2560px.
...
⚠️ **Todas las secciones van dentro de `.container` (1400 px como máximo, centrado) con el margen lateral de `.section`.** Las tres únicas que llegan de borde a borde a propósito son la portada (`#home`), el carrusel de fotos (`#academy`, que necesita el ancho completo para las flechas laterales) y el mapa (`#location`). `#instructors` **no** es una de ellas desde el 7 de septiembre de 2026: iba a pantalla completa por un `max-width: 100%` y llegaba pegado a los bordes.

⚠️ **Para juzgar si una sección está centrada, mide el TEXTO VISIBLE, no la caja.** El contenedor puede estar perfectamente centrado y aun así verse descuadrado: en `#about` el texto de la izquierda llegaba al borde del contenedor mientras el poema, centrado verso a verso dentro de una columna de 660px, dejaba 140px de aire a cada lado — y ese aire hacía el margen derecho mucho mayor que el izquierdo. Se arregló dando a `#itaca-poem` el ancho de su verso más largo (`max-width: 25rem`) y pegándolo a la derecha con `margin-left: auto`. Recorre los nodos de texto con `Range.getClientRects()` para sacar el extremo real; **descarta los que tengan `opacity: 0`** (la firma del poema empieza invisible y falsea la medida). Ojo también con comparar el borde izquierdo de un bloque con el derecho de otro distinto: un párrafo alineado a la izquierda cuya última línea queda corta parece descuadre y no lo es.

⚠️ **El ancho del poema es fijo a propósito, no `fit-content`.** El poema se escribe letra a letra (`typed += poemText[poemIndex]`), así que una caja que se adaptara al contenido crecería mientras teclea. Si cambias el poema o su tipografía, vuelve a medir el verso más largo y ajusta el `max-width`; si se queda corto, los versos se parten.

⚠️ **Las tipografías de Google no cargan en el entorno del agente**, así que cualquier medida de anchura de texto sale con la fuente de reserva y es falsa. Para medir de verdad: descarga los `.woff2` con `curl` y sírvelos al navegador de pruebas con `page.route()` interceptando `fonts.googleapis.com` y `fonts.gstatic.com` (hay un ayudante en el scratchpad de la sesión del 7 de septiembre de 2026). Y fuerza `document.fonts.load('300 18px "Crimson Pro"')` antes de medir: una fuente que aún no se ha usado en la página no está cargada aunque `document.fonts.ready` haya resuelto.

⚠️ **Un comentario CSS sin cerrar se comió una `@media` entera durante meses.** En `index.html` había un `/*` abierto justo antes de `@media (max-width: 768px) {`: el comentario se tragaba la línea de la media query, así que las reglas del carrusel deslizable de instructores (`display: flex`, fichas de 320 px) se aplicaban **en todos los anchos**, y en un monitor grande los tres instructores salían amontonados a la izquierda. Se ve a simple vista en el navegador pero no leyendo el CSS. **Para comprobarlo, recorre `document.styleSheets` y mira el `conditionText` de cada regla**: si una regla que debería ser de móvil sale sin media query, hay un comentario roto por encima.
...
⚠️ **`.reels-grid` necesita `width: auto` en la fila deslizable de móvil.** Su regla base lleva `width: 100%`, que clava el ancho al del contenedor: el `margin-inline` negativo movía la fila a la izquierda pero no la ensanchaba, así que se quedaba a un gutter del borde derecho mientras la de instructores sí llegaba. Se ve como un hueco blanco a la derecha de los vídeos.

⚠️ **El tamaño de las fichas deslizables de móvil está elegido para que se vea que hay más.** Instructores a `62vw` (entra uno y medio). A ficha casi de pantalla completa cada una parecía una página aparte y nadie descubría que se deslizan. **Los reels NO se deslizan en móvil**: van en rejilla de 2×2, porque son cuatro y entran justos en dos filas; deslizándose se veía "uno y medio" con el segundo partido por la mitad.

⚠️ **Instructores en `#instructors` va en rejilla de 2 columnas en móvil, con Cipri siempre grande arriba y el resto en parejas debajo.** Cipri pidió (13 de septiembre de 2026) mejorar cómo se veían mientras solo hay 3 profesores; al día siguiente afinó: *"la foto grande debe ser la de Cipri, como head coach, y Boris y Marta más pequeño"*.

Solución en dos reglas:
- `.instructors-grid .instructor-card:first-child { grid-column: 1 / -1; aspect-ratio: 3/4; }` — el primero de la lista (Cipri, es el head coach; el orden importa, lo marca el HTML) ocupa siempre la fila entera, con su propio `aspect-ratio` menos alto que el de las fichas pequeñas (a todo el ancho del teléfono, `3/4.6` quedaría desproporcionado).
- `.instructors-grid .instructor-card:last-child:nth-child(even) { grid-column: 1 / -1; }` — si el último profesor de la lista se queda sin pareja, ocupa la fila entera en vez de dejar un hueco vacío al lado. Es `(even)` y no `(odd)` **porque Cipri ya ocupa la posición 1 él solo**: con 3 fichas totales el último está en posición 3 (impar) y no sobra nadie (Boris y Marta se emparejan); con 4 fichas el último está en posición 4 (par) y sí sobra, así que se ensancha.

**Esto es automático y no hay que tocarlo al añadir o quitar profesores** (mientras el primero de la lista siga siendo Cipri). Si algún día cambia quién es el head coach, mueve su ficha a la primera posición en el HTML — la regla `:first-child` lo recoge solo.

⚠️ **La ficha de 2 columnas es más estrecha que las de escritorio o tablet, y `.instructor-content` se centra en vertical** (`top:50%; transform:translate(-50%,-50%)`) sobre una ficha con `overflow:hidden`. Si el bloque de texto (rol + nombre + biografía) no cabe en la altura de la ficha, se recorta por arriba y por abajo a la vez **sin avisar** — pasó de verdad con la biografía de Boris, la más larga de las tres, que se recortaba a partir de 430px de ancho. Por eso la rejilla de móvil lleva su propio `aspect-ratio: 3/4.6` (más alta que el `3/4` de escritorio) y letra más pequeña (`h3` a `clamp(1.6rem, 7vw, 2.2rem)`, `p` a `0.82rem`). **Si se reescribe una biografía y se alarga, vuelve a comprobar que no se recorta** — mide con `getBoundingClientRect()` del `<p>` contra la ficha, no te fíes a ojo: en una ficha con overflow:hidden el recorte no se nota mirando el código, solo abriendo la web.

⚠️ **Para comprobar el texto que solo se ve en móvil (`.instructor-card p`, oculto tras `:hover` en escritorio), el navegador de pruebas necesita `isMobile: true` y `hasTouch: true`.** Sin eso, Playwright reporta `(hover: hover) and (pointer: fine)` como verdadero aunque el ancho de ventana sea de móvil, y el CSS esconde el párrafo (`opacity: 0`) igual que en un ratón de verdad — parece un fallo ("la biografía no se ve") y es solo la prueba mal montada. La posición y el tamaño (`getBoundingClientRect`) no cambian con la opacidad, así que las medidas de si algo se recorta siguen siendo válidas sin `isMobile`; lo que no vale sin `isMobile` es una captura de pantalla para enseñar cómo se ve de verdad.
