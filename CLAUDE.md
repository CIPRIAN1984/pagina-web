...
| Panel de solicitudes | `panel/index.html` | Uso interno de Cipri, no la web pública: tarjetas con quién ha escrito, botón de WhatsApp y qué acción tomar (mandar info / apuntar a prueba / otro), sin tener que abrir la hoja de cálculo. `noindex` + `Disallow` en `robots.txt`, protegido por una clave (`CLAVE_PANEL`) que solo vive en el script de Google, nunca en el repositorio. Lee y escribe contra el mismo Apps Script que guarda los leads — ver `docs/hoja-de-calculo.md`. |
