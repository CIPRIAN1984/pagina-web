...
| Fotos del gimnasio | `images/Itaca/Gym1.webp` … `Gym10.webp` (10) | Las lee el array `photos` del JS de `index.html` y el array `FOTOS` de `movil.html`. Si añades una foto, hay que añadirla en los dos sitios. En escritorio solo las 8 primeras salen en el carrusel (`CAROUSEL_MAX`); todas salen en "Ver todas las fotos". `Gym8-10.webp` son nuevas (6 de septiembre de 2026, fotos de Drive recortadas a 1200×800; `Gym9` y `Gym10` llevan cara de una menor con permiso firmado de la familia, confirmado por Cipri). |
